...
@Composable fun PixelPy() {
    ...
    val runtimeMutex = remember { Mutex() }
    ...
    fun runCode(debug: Boolean = false) {
        ...
        current.writeText(code.text); backupFile(current); running = true; runtimeBusy = true; tab = Tab.Console
        val executedFile = current.name
        val executedSource = code.text
        val executedProject = dir.absolutePath
        lastRunFile = executedFile; lastRunSource = executedSource
        ...
        scope.launch {
            try {
                ...
                running = false; runtimeBusy = false; inputBridge = null
            }
        }
    }
        ...
        Scaffold(containerColor = Paper, topBar = {
            ...
            Box(Modifier.padding(padding).fillMaxSize()) {
                when (tab) {
                    ...
                    Tab.Repl -> ReplScreen(dir, replEntries, runtimeMutex, runtimeBusy, { runtimeBusy = it }) { replEntries = it }
                    Tab.Console -> Console(lastRunFile, lastRunSource, output, generated, history, success, running, lastErrorLine, onBack = { tab = Tab.Editor }, onGoToLine = { line ->
                        val target = File(dir, lastRunFile)
                        val position = lastRunSource.lineSequence().take((line - 1).coerceAtLeast(0)).sumOf { it.length + 1 }.coerceAtMost(lastRunSource.length)
                        if (target.exists()) current = target
                        code = TextFieldValue(lastRunSource, selection = TextRange(position)); tab = Tab.Editor
...
@Composable private fun ReplScreen(project: File, entries: List<ReplEntry>, runtimeMutex: Mutex, runtimeBusy: Boolean, onRuntimeBusy: (Boolean) -> Unit, onEntries: (List<ReplEntry>) -> Unit) {
    ...
    fun evaluate() {
        ...
        running = true; onRuntimeBusy(true); command = ""
        scope.launch {
            try {
                val result = withContext(Dispatchers.Default) {
                    runtimeMutex.withLock {
                ...
                running = false; onRuntimeBusy(false)
            }
        }
    }
