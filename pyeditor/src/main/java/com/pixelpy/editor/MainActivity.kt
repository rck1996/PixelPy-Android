    ...
    fun runCode(debug: Boolean = false) {
        ...
        scope.launch {
            val result = withContext(Dispatchers.Default) {
                ...
                val issues = module.callAttr("analyze", code.text).asList().map { it.toString() }
                val errors = issues.filter { it.startsWith("ERROR|") }
                if (errors.isNotEmpty()) Triple(false, "ANÁLISIS PREVIO\n\n" + issues.joinToString("\n") { issue -> val parts = issue.split('|', limit = 3); "${parts[0]} · línea ${parts[1]}: ${parts[2]}" }, emptyList())
                else {
                    val value = module.callAttr("execute", code.text, "", dir.absolutePath, bridge, 120, debug)
                    ...
                    Triple(value.callAttr("get", "ok").toBoolean(), (if (warnings.isBlank()) "" else "$warnings\n\n") + value.callAttr("get", "output").toString() + if (trace.isBlank()) "" else "\n\nDEPURACIÓN · LÍNEAS Y VARIABLES\n$trace", value.callAttr("get", "files").asList().map { File(it.toString()) })
                }
            }
            success = result.first; output = result.second.ifBlank { "✓ Programa terminado sin salida." }; generated = result.third; if (result.third.isNotEmpty()) resourceVersion++; running = false
            history = (listOf(RunRecord(current.name, result.first, output.take(700), System.currentTimeMillis())) + history).take(20)
        ...
        Scaffold(containerColor = Paper, topBar = {
            ...
            Box(Modifier.padding(padding).fillMaxSize()) {
                when (tab) {
                    ...
                    Tab.Repl -> ReplScreen(dir, replEntries) { replEntries = it }
                    Tab.Console -> Console(current.name, code.text, output, generated, history, success, running, onBack = { tab = Tab.Editor }, onGoToLine = { line ->
                        val position = code.text.lineSequence().take((line - 1).coerceAtLeast(0)).sumOf { it.length + 1 }.coerceAtMost(code.text.length)
                        code = code.copy(selection = TextRange(position)); tab = Tab.Editor
...
@Composable private fun ReplScreen(project: File, entries: List<ReplEntry>, onEntries: (List<ReplEntry>) -> Unit) {
    var command by rememberSaveable(project.path) { mutableStateOf("") }
    var running by remember { mutableStateOf(false) }
    val scope = rememberCoroutineScope()
    fun evaluate() {
        val source = command.trim(); if (source.isEmpty() || running) return
        ...
        scope.launch {
            ...
            onEntries((entries + ReplEntry(source, result.second.ifBlank { "✓" }, result.first)).takeLast(100)); running = false
        }
    }
    Column(Modifier.fillMaxSize().imePadding().padding(14.dp)) {
        Text("PYTHON REPL", fontWeight = FontWeight.Black, fontSize = 27.sp)
        Text("Sesión activa · ${project.name}", fontFamily = FontFamily.Monospace, fontSize = 12.sp)
        ...
        Spacer(Modifier.height(10.dp)); Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OutlinedTextField(command, { command = it }, Modifier.weight(1f), label = { Text(">>> comando") }, enabled = !running, textStyle = TextStyle(fontFamily = FontFamily.Monospace), maxLines = 4)
            BrutalButton(if (running) "…" else "EJECUTAR", Green, enabled = !running, onClick = ::evaluate)
        }
...
@Composable private fun Console(fileName: String, code: String, output: String, generated: List<File>, history: List<RunRecord>, success: Boolean?, running: Boolean, onBack: () -> Unit, onGoToLine: (Int) -> Unit, onRun: () -> Unit, onSave: (File) -> Unit, onShare: (File) -> Unit) {
    val context = androidx.compose.ui.platform.LocalContext.current
    val clipboard = androidx.compose.ui.platform.LocalClipboardManager.current
    val report = "PIXELPY LOG\n\nCÓDIGO:\n$code\n\nSALIDA:\n$output"
    val errorLine = Regex("File \\\"main\\.py\\\", line (\\d+)").find(output)?.groupValues?.getOrNull(1)?.toIntOrNull()
